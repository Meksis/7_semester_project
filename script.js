...
// let url = 'proxy51.rt3.io:39571';
let server_url = '127.0.0.1:5000';

let proto = 'http';
let url = 'zitrax-ollama-11434-tcp.at.remote.it:33001';
...
(async () => {
  ...
  if (uuid) {
    ...
    const response = await fetch(`${proto}://${server_url}/get_chats4user?uuid=${uuid}`, {
      method: 'GET',
    ...
    const response = await fetch(`${proto}://${server_url}/create_user`, {
      method: 'POST',
      headers: {
...
async function sendMessage() {
  ...
  if (userInput.value.trim()) {
    ...
    try {
      const response = await fetch(`${proto}://${url}/api/chat`, {
        // const response = await fetch(`${proto}://${url}/api/chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
